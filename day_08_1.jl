  ...
  for antenna_locations in values(antennas)
    for antenna1 in antenna_locations
      for antenna2 in antenna_locations
        ...
        resonance1 = add_distances(antenna1, distance)
        resonance2 = add_distances(antenna2, flip_distance(distance))

        if is_in_bounds(resonance1, bound_x, bound_y)
          push!(solutions, resonance1)
        end
        if is_in_bounds(resonance2, bound_x, bound_y)
