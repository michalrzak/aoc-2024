function main()
  ...
  parsed_ints = map(x -> Dict(var => parse(Int64, x[var]) for var in variables), parsed)

  presses = map(x ->
      begin
        b = (x[:result2] - (x[:result1] // x[:a1] * x[:b1])) // (-x[:a2] // x[:a1] * x[:b1] + x[:b2])
        a = (x[:result1] // x[:a1] - x[:a2] // x[:a1] * b)
        (a, b)
      end, parsed_ints)
